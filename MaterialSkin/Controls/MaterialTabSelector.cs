...
namespace MaterialSkin.Controls
{
    public class MaterialTabSelector : Control, IMaterialControl
    {
        ...
        public List<int> badgeNums;  //角标数字
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            foreach (TabPage tabPage in baseTabControl.TabPages)
            {
                ...
                if (badgeNums[currentTabIndex] > 0)
                {
                    ...
                    string badgeText;
                    if (badgeNums[currentTabIndex] > 99)
                    {
                        badgeText = "..";
                    }
                    else
                    {
                        badgeText = string.Format("{0}", badgeNums[currentTabIndex]);
                    }
                    g.DrawString(badgeText, SkinManager.ROBOTO_MEDIUM_10, textBrush, rect, format);
                }
        ...
        private void UpdateTabRects()
        {
            ...
            using (var b = new Bitmap(1, 1))
            {
                using (var g = Graphics.FromImage(b))
                {
                    ...
                    tabRects.Add(new Rectangle(SkinManager.FORM_PADDING, 0, TAB_HEADER_PADDING * 2 + (int)textSize.Width, Height));
