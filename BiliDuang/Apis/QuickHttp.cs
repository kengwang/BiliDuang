...
/// <summary>
    ...
    {
        public static async Task<byte[]> SendAsync(object url, object method, object headers = null,
            object content = null, bool ensureSuccessStatusCode = true, Out<HttpStatusCode> statusCode = null,
            Out<CookieCollection> setCookie = null, CancellationToken cancellationToken = default)
        {
        ...
        public static async Task<HttpResponseMessage> SendAsync(this HttpClient client, object url, object method,
            object headers = null, object content = null, CancellationToken cancellationToken = default)
        {
            if (client is null)
            ...
            if (!(url is Uri) && !(url is string))
                throw new ArgumentOutOfRangeException(nameof(url),
                    $"For '{url}', only the following types are supported: Uri, string");
            if (method is null)
                throw new ArgumentNullException(nameof(method));
            if (!(method is HttpMethod) && !(method is string))
                throw new ArgumentOutOfRangeException(nameof(method),
                    $"For '{method}', only the following types are supported: HttpMethod, string");

            using var request = new HttpRequestMessage(
                method is string methodString ? new HttpMethod(methodString) : (HttpMethod)method,
                url is string urlString ? new Uri(urlString) : (Uri)url);
            if (!(content is null) && !ContentConverters.TryConvert(request, content))
                throw new ArgumentOutOfRangeException(nameof(content),
                    $"For '{content}', only the following types are supported: {ContentConverters.SupportedTypesString}");
            if (!(headers is null) && !HeaderConverters.TryConvert(request, headers))
                throw new ArgumentOutOfRangeException(nameof(headers),
                    $"For '{headers}', only the following types are supported: {HeaderConverters.SupportedTypesString}");
        ...
        private static class HeaderConverters
        {
            ...
            public static readonly string SupportedTypesString =
                string.Join(", ", Converters.Select(t => t.TypeString));
            ...
            private static void Convert(HttpRequestMessage request,
