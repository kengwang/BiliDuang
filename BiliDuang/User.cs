...
namespace BiliDuang
{
    public class User
    {
        ...
        public static string face
        {
            ...
            set
            {
                ...
                if (!File.Exists(deerory + fileName))
                {
                    ...
                    {
                        System.Drawing.Image downImage =
                            System.Drawing.Image.FromStream(imgRequest.GetResponse().GetResponseStream());
        ...
        public static JSONCallback.User.User GetUserInfo(string uid)
        {
            ...
            try
            {
                string DataRaw = Encoding.UTF8.GetString(
                    MyWebClient.DownloadData("https://api.bilibili.com/x/space/acc/info?mid=" + uid + "&jsonp=jsonp"));
                MyWebClient.Dispose();
        ...
        public static void RefreshUserAcessKey()
        {
            ...
            try
            {
                ...
                JSONCallback.ThirdLogin.ThirdLogin thirdlogin =
                    JsonConvert.DeserializeObject<JSONCallback.ThirdLogin.ThirdLogin>(callback);
                if (thirdlogin.status == true && thirdlogin.data.has_login == 1 &&
                    thirdlogin.data.user_info.mid.ToString() == uid)
                {
                    request = HttpWebRequest.CreateHttp(thirdlogin.data.confirm_uri);
                    request.AllowAutoRedirect = false;
                    request.Headers.Add("Cookie", cookie);
                    access_key = Other.TextGetCenter("access_key=", "&",
                        request.GetResponse().Headers[HttpResponseHeader.Location]);
                }
                else
                {
                    // 换取失败
        ...
        public static void RefreshUserInfo()
        {
            ...
            {
                UserDataFile uf =
                    JsonConvert.DeserializeObject<UserDataFile>(
                        File.ReadAllText(Environment.CurrentDirectory + "/config/user"));
                uid = uf.uid;
            ...
            try
            {
                UserDataRaw =
                    Encoding.UTF8.GetString(
                        MyWebClient.DownloadData("https://api.bilibili.com/x/space/myinfo?jsonp=jsonp"));
